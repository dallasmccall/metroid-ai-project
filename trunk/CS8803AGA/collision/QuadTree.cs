    ...
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        private void ExpandRoot(DoubleRect newChildBounds)
        {
            lock (syncLock)
            {
                ...
                DiagDirection rootDirection;
                if (isNorth)
                {
                    rootDirection = isWest ? DiagDirection.NW : DiagDirection.NE;
                }
                else
                {
                    rootDirection = isWest ? DiagDirection.SW : DiagDirection.SE;
                }

                double newX = (rootDirection == DiagDirection.NW || rootDirection == DiagDirection.SW)
                                  ? root.Bounds.X
                                  : root.Bounds.X - root.Bounds.Width;
                double newY = (rootDirection == DiagDirection.NW || rootDirection == DiagDirection.NE)
                                  ? root.Bounds.Y
        ...
        private void SetupChildNodes(QuadNode node)
        {
            lock (syncLock)
            {
                ...
                {
                    node[DiagDirection.NW] = new QuadNode(node.Bounds.X, node.Bounds.Y, node.Bounds.Width / 2,
                                                      node.Bounds.Height / 2);
                    node[DiagDirection.NE] = new QuadNode(node.Bounds.X + node.Bounds.Width / 2, node.Bounds.Y,
                                                      node.Bounds.Width / 2,
                                                      node.Bounds.Height / 2);
                    node[DiagDirection.SW] = new QuadNode(node.Bounds.X, node.Bounds.Y + node.Bounds.Height / 2,
                                                      node.Bounds.Width / 2,
                                                      node.Bounds.Height / 2);
                    node[DiagDirection.SE] = new QuadNode(node.Bounds.X + node.Bounds.Width / 2,
        ...
        private void CheckChildNodes(QuadNode node)
        {
            lock (syncLock)
            {
                if (GetQuadObjectCount(node) <= maxObjectsPerLeaf)
                {
                    ...
                    if (node[DiagDirection.NW] != null)
                    {
                        node[DiagDirection.NW].Parent = null;
                        node[DiagDirection.NW] = null;
                    }
                    if (node[DiagDirection.NE] != null)
                    {
                        node[DiagDirection.NE].Parent = null;
                        node[DiagDirection.NE] = null;
                    }
                    if (node[DiagDirection.SW] != null)
                    {
                        node[DiagDirection.SW].Parent = null;
                        node[DiagDirection.SW] = null;
                    }
                    if (node[DiagDirection.SE] != null)
                    {
                        node[DiagDirection.SE].Parent = null;
                        node[DiagDirection.SE] = null;
                    }
        ...
        public class QuadNode
        {
            ...
            public QuadNode this[DiagDirection direction]
            {
                get
                {
                    switch (direction)
